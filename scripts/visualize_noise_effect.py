...
def denormalize(batch_data, device):
    ...
    """
    batch_data = batch_data * STD_CHANNEL.unsqueeze(0).unsqueeze(-1).unsqueeze(
        -1
    ).unsqueeze(-1).to(device) + MEAN_CHANNEL.unsqueeze(0).unsqueeze(-1).unsqueeze(
        -1
...
def main():
    ...
    plt.tight_layout()
    plt.savefig(
        "/workspace/meteolibre_model/noise_effect.png", dpi=300, bbox_inches="tight"
    )
    print(
        "Noise effect visualization saved to /workspace/meteolibre_model/noise_effect.png"
    )

    # Print some statistics
    print("\nStatistics for Channel 0:")
    print(
        f"Original (t=0): mean={x_noisy_denorm_original[0, 0, 0].mean().item():.2f}, std={x_noisy_denorm_original[0, 0, 0].std().item():.2f}"
    )
    for t in t_values[1:]:
        t_tensor = torch.tensor(t, device=device)
        sigma_sq = sigma_t_sq(t_tensor)
        ...
        x_noisy_denorm = denormalize(x_noisy, device)
        print(
            f"At t={t}: mean={x_noisy_denorm[0, 0, 0].mean().item():.2f}, std={x_noisy_denorm[0, 0, 0].std().item():.2f}"
        )
