...
# Load config for v3 which includes correction model params
...
@torch.no_grad()
def tiled_correction_inference(
    model,
    initial_context,  # (B, C, 2, H_big, W_big)
    initial_x,  # (B, C, 1, H_big, W_big) the candidate to correct
    patch_size,
    steps,
    device,
    date,
    ...
    patch_coords = patch_coords1 + patch_coords2
    d_const = 1.0 / steps
    for i in tqdm(range(steps), desc="Tiled Correction"):
        t_val = 1.0 - i * d_const
        t_batch_val = torch.full((1,), t_val, device=device)
        d_batch_val = torch.full((1,), d_const, device=device)
        aggregated_pred = torch.zeros_like(x_t_full_res, device=device)
        ...
        x_t_full_res = x_t_full_res - averaged_pred * d_const
        x_t_full_res = x_t_full_res.clamp(-7, 7)
    # For correction model, no adding last context (full frame prediction)
    # Clamp to normalized range
    x_t_full_res = torch.where(
        x_t_full_res < CLIP_MIN, CLIP_MIN, x_t_full_res
    )
    x_t_full_res = x_t_full_res.clamp(-7, 7)
    return x_t_full_res.cpu()
...
def main():
    parser = argparse.ArgumentParser(
        description="Tiled Diffusion for large image forecasting with Shortcut Rectified Flow and Correction."
    )
    parser.add_argument(
        "--model_path",
        type=str,
        default="models/20251019_epoch61_mtg_lightning_soap.safetensors",
        help="Path to the pre-trained forecast model .safetensors file.",
    )
    parser.add_argument(
        "--correction_model_path",
        type=str,
        default=None,
        help="Path to the pre-trained correction model .safetensors file (optional).",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="generated_forecasts_shortcut_correction",
    ...
    parser.add_argument(
        ...
        help="Number of context frames expected by the forecast model.",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    # Load forecast model weights
    if os.path.exists(args.model_path):
        loaded_state_dict = load_file(args.model_path)
        forecast_model.load_state_dict(loaded_state_dict)
        print(f"Loaded forecast model weights from {args.model_path}")
    else:
        print(
            f"Warning: Forecast model weights not found at {args.model_path}. Using randomly initialized model."
        )
    forecast_model.to(args.device)
    # Initialize correction model if path provided
    correction_model = None
    if args.correction_model_path:
        correction_params = params["model_correction"]
        correction_model = DualUNet3DFiLM(**correction_params)
        if os.path.exists(args.correction_model_path):
            loaded_state_dict = load_file(args.correction_model_path)
            correction_model.load_state_dict(loaded_state_dict)
            print(f"Loaded correction model weights from {args.correction_model_path}")
            correction_model.to(args.device)
        else:
            print(f"Warning: Correction model weights not found at {args.correction_model_path}. Skipping correction.")
            correction_model = None
    ...
    for step_idx in range(args.forecast_steps):
        ...
        prediction_date = initial_date + timedelta(minutes=10 * (step_idx + args.context_frames))
        # Perform tiled inference for the next frame (forecast)
        ...
        )  # Shape: (1, nb_channels, 1, H_big, W_big) normalized
        generated_frame = generated_frame.to(args.device)
        # Apply correction if available
        if correction_model is not None:
            # Construct correction context: last two frames
            correction_context = current_high_res_context[:, :, -2:, :, :]  # (1, C, 2, H, W)
            # Run tiled correction inference, starting from generated_frame as initial_x
            corrected_frame = tiled_correction_inference(
                model=correction_model,
                initial_context=correction_context,
                initial_x=generated_frame.unsqueeze(2),  # Add time dim: (1, C, 1, H, W)
                patch_size=args.patch_size,
                steps=args.denoising_steps,
                device=args.device,
                date=prediction_date,
            ...
            corrected_frame = corrected_frame.to(args.device)
            frame_to_use = corrected_frame
            print("Applied correction to the generated frame.")
        else:
            frame_to_use = generated_frame.unsqueeze(2)
        # Split and denormalize the frame to use
        sat_generated = frame_to_use[:, :c_sat]
        lightning_generated = frame_to_use[:, c_sat:]
        sat_generated, lightning_generated = denormalize(
            sat_generated, lightning_generated, args.device
        )
        # Save the generated/corrected frame (sat and lightning channels)
        ...
        # Shift the context: remove the oldest frame, add the newly generated/corrected frame (normalized)
        frame_to_use_normalized = frame_to_use.squeeze(2)  # Remove time dim
        current_high_res_context = torch.cat(
            [
                current_high_res_context[:, :, 1:, :, :],
                frame_to_use_normalized.unsqueeze(2),
            ],
            dim=2,
        )  # (1, nb_channels, 4, H_big, W_big)
    print("Tiled diffusion and autoregressive forecasting with correction complete.")
