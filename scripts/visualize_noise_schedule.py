...
def main():
    ...
    print("Noise schedule visualization saved to /workspace/meteolibre_model/noise_schedule.png")

    # Also print some values
    print(f"At t=0: beta={beta_t(torch.tensor(0.0)):.4f}, sigma_sq={sigma_t_sq(torch.tensor(0.0)):.4f}")
    print(f"At t=0.5: beta={beta_t(torch.tensor(0.5)):.4f}, sigma_sq={sigma_t_sq(torch.tensor(0.5)):.4f}")
    print(f"At t=1: beta={beta_t(torch.tensor(1.0)):.4f}, sigma_sq={sigma_t_sq(torch.tensor(1.0)):.4f}")
