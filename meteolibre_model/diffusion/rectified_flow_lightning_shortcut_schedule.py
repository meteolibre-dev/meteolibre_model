...
Modified to use a cubic noise schedule for improved generative quality on large images:
x_t = (1 - t)^3 * x0 + (1 - (1 - t)^3) * x1
With corresponding adjustment to the velocity target in training.
Inspired by https://arxiv.org/abs/2301.11093
"""
import torch
import math
import random
...
def get_x_t_rf(x0, x1, t):
    """
    Get the interpolated point x_t = (1 - t)^3 * x0 + (1 - (1 - t)^3) * x1
    """
    u = 1 - t
    alpha = u ** 3
